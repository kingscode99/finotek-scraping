...
import com.hanulplc.customer.woori.WooriProductCode;
...
@Slf4j
public class WooriCaseDetailPage {
    ...
    public void navigateToFirstTab() {
        ...
        $("#applDto > div:nth-child(55) > h3").shouldBe(visible);
        // =======================================================
    }
    ...
    public Map<String, String> 등기_의뢰_정보(String caseManagementNumber) {
        ...
        String 금융기관신청번호 = $("#applDto > div:nth-child(55) > table > tbody > tr:nth-child(2) > td:nth-child(2)").text();
        String 지점명 = $("#eai_fncl_br_nm").getValue();
        String 담당자명 = $("#applDto > div:nth-child(55) > table > tbody > tr:nth-child(3) > td:nth-child(2)").text();
        String 담당자_연락처 = $("#applDto > div:nth-child(55) > table > tbody > tr:nth-child(3) > td:nth-child(4)").text();
        String 담당자_팩스번호 = $("#applDto > div:nth-child(55) > table > tbody > tr:nth-child(3) > td:nth-child(6)").text();

        log.info("금융기관명 = {}", 금융기관명);
        log.info("금융기관신청번호 = {}", 금융기관신청번호);
        log.info("지점명 = {}", 지점명);
        log.info("담당자명 = {}", 담당자명);
        ...
        caseBasicInfo.put("ACCNT_CD", "WRB");
        caseBasicInfo.put("RGSTRN_REQ_NUM", 의뢰번호);
        caseBasicInfo.put("RGSTRN_BANK_NUM", 금융기관신청번호);
    ...
    public Map<String, String> 등기_상세_정보() {
        String 전자등기구분 = $("#applDto > div:nth-child(56) > table > tbody > tr:nth-child(1) > td:nth-child(2) > input").getValue();
        String 등기유형 = $("#rgst_type").getSelectedText().replace(" ", "");
        String 등기원인 = $("#slt_rgst_caus").getSelectedText();
        String 등기원인_연월일 = $("#rgst_caus_date").getValue(); // 설정계약일자
        String 등기희망일자 = $("#rgst_caus_date").getValue();
        String 대출실행일자 = $("#loan_exe_date").getValue();
        String 채권최고액 = $("#applDto > div:nth-child(56) > table > tbody > tr.item_2810.hide > td:nth-child(2) > input.price.readonly").getValue();
        ...
        caseDetailInfo.put("RGSTRN_TYP_DETAIL", WooriProductCode.of(전자등기구분).getCode());
    ...
    public List<Map<String, String>> 담보제공자_정보(String caseManagementNumber) {
        ...
        log.info("담보제공자(등기의무자) 수 = {}명", securityProviders.size());

        List<Map<String, String>> securityProvidersInfos = new ArrayList<>();
        for (SelenideElement securityProvider : securityProviders) {
            String 채무자_동일_여부 = securityProvider.find(By.tagName("td"), 0).find("input[type='checkbox']").isSelected() ? "동일" : "";
            String 담보제공자_성명 = securityProvider.find(By.tagName("td"), 1).text();
            String 담보제공자_주소 = securityProvider.find(By.tagName("td"), 5).text();

            if (담보제공자_성명.equals(채무자_성명)) {
                채무자_동일_여부 = "동일";
                isSecurityProviderEqualsToMortgager = true;
            }

            securityProvider.find(By.tagName("td"), 1).click();
            switchTo().frame($("iframe.popupIframe"));

            String 담보제공자_주민등록번호 = $("#oblg_corp_no").val().replace("-", "");
            String 담보제공자_연락처 = $("#oblg_hpn_no").val();

            $("body > div.txt_cen > input:nth-child(2)").click();
            switchTo().defaultContent();

            log.info("채무자 동일 여부 = {}", 채무자_동일_여부);
            log.info("담보제공자(등기의무자) 성명 = {}", 담보제공자_성명);
            log.info("담보제공자(등기의무자) 주민등록번호 = {}", 담보제공자_주민등록번호);
            log.info("담보제공자(등기의무자) 연락처 = {}", 담보제공자_연락처);
            log.info("담보제공자(등기의무자) 주소 = {}", 담보제공자_주소);
            ...
            if (securityProviders.size() > 1) { // 담보제공자 정보에 2명 이상이 존재하는 경우
                if (채무자_동일_여부.equals("동일")) { // 현재 담보제공자가 채무자와 동일한 경우, 채무자로 저장
                    ...
                    채무자.put("CUST_TEL_NUM_1", 담보제공자_연락처);
                    ...
                    설정자.put("CUST_TEL_NUM_1", 담보제공자_연락처);
    ...
    public List<Map<String, String>> 부동산_정보(String caseManagementNumber) {
        ...
        for (SelenideElement realEstate : realEstates) {
            ...
            String 관할등기소 = elements.get(6).getText();
            String 부동산_주소 = elements.get(7).getText();
