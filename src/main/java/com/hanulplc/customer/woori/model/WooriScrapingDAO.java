...
@Slf4j
public class WooriScrapingDAO {

    private WooriScrapingDAO() {
    }
    ...
    public static OptionalInt findByCaseId(String caseId) {
        try (SqlSession session = getSession()) {
            WooriMapper mapper = session.getMapper(WooriMapper.class);
            return OptionalInt.of(mapper.findByCaseId(caseId));
        } catch (Exception e) {
            log.error("우리은행 의뢰 조회 중 예외 발생", e);
    ...
    public static void saveNewCase(Map<String, String> caseMap,
        ...
        try {
            ...
            log.error("우리은행 의뢰 추가 중 예외 발생", e);
            session.rollback();
    ...
    private static void 설정_고객정보_입력(Map<String, String> caseMap, List<Map<String, String>> customers, WooriMapper mapper) {
    ...
    private static void 구입자금일_경우_이전_고객정보_입력(Map<String, String> caseMap, List<Map<String, String>> customers, WooriMapper mapper) {
