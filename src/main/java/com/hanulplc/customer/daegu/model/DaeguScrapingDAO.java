package com.hanulplc.customer.daegu.model;
...
@Slf4j
public class DaeguScrapingDAO {

    private DaeguScrapingDAO() {
    }

    private static SqlSession getSession() {
        return MybatisSessionFactory.newProdSession();
    }

    public static OptionalInt findByCaseId(String caseId) {
        try (SqlSession session = getSession()) {
            DaeguMapper mapper = session.getMapper(DaeguMapper.class);
            return OptionalInt.of(mapper.findByCaseId(caseId));
        } catch (Exception e) {
            log.error("대구은행 의뢰 조회 중 예외 발생", e);
    ...
    public static void saveNewCase(Map<String, String> caseMap,
        ...
        try {
            ...
            log.error("대구은행 의뢰 추가 중 예외 발생", e);
            session.rollback();
    ...
    private static void 설정_고객정보_입력(Map<String, String> caseMap, List<Map<String, String>> customers, DaeguMapper mapper) {
        for (Map<String, String> customer : customers) {
            log.info("설정 고객정보 입력 = {}", customer);
            mapper.saveNewCustomer(customer);

            // '구입자금', '자담'일 경우, 등기의무자가 1명이라면 채무자가 설정자이기도 함.
            if (customers.size() == 1 &&
                    customer.get("CUST_CL_CD").equals("1") &&
    ...
    private static void 구입자금일_경우_이전_고객정보_입력(Map<String, String> caseMap, List<Map<String, String>> customers, DaeguMapper mapper) {
