...
import com.hanulplc.customer.nonghyup.nh.model.NonghyupNHScrapingDAO;
...
@Slf4j
public class NonghyupNHCaseListPage implements FinotekCaseListPage {

    public static final String URL = "https://nhers.nonghyupit.com:8100/rgst/appl/list";

    @Override
    public FinotekCaseListPage searchByConditions() {
        // '기준일자'를 '대출실행일자'로 선택
        $("select[name='sc']").shouldBe(enabled).selectOption("대출실행일자");

        String dayBeforeTwoWeeks = LocalDate.now().minusWeeks(2).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        String dayAfterMonth = LocalDate.now().plusMonths(4).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        executeJavaScript("$('#" + $("#date_start").getAttribute("id") + "').val('" + dayBeforeTwoWeeks + "');");
        executeJavaScript("$('#" + $("#date_end").getAttribute("id") + "').val('" + dayAfterMonth + "');");
...
//        $("#cm9010").selectOption(0);
    ...
    public FinotekCaseListPage scrapeCases() {
        ...
        for (String caseId : caseIds) {
            OptionalInt count = NonghyupNHScrapingDAO.findByCaseId(caseId);
    ...
    private void scrapeCase(String caseId) {
        NonghyupNHCaseDetailPage caseDetailPage = open(
            String.format("https://nhers.nonghyupit.com:8100/rgst/appl/edit/%s", caseId),
            NonghyupNHCaseDetailPage.class
        );
        ...
        NonghyupNHScrapingDAO.saveNewCase(의뢰_기본_정보, 담보제공자_정보, 부동산_정보);
    }
}
