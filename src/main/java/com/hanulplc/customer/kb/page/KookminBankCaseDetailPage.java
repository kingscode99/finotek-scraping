...
@Slf4j
public class KookminBankCaseDetailPage {

    public void navigateToFirstTab() {
        // =======================================================
        // 이미 진행된 건에 대하여 '1. 등기 정보 검수' 탭으로 이동
        if (!$("#progStepDiv1 > nav > div > ul > li.line_arrow.active > span").exists()) {
            $("#progStepDiv1 > nav > div > ul > li.line_on > span").click();
        }
    ...
    public String 대환유무() {
        return "1".equals($("#dh_yn").val()) ? "유" : "무";
    }

    public Map<String, String> 등기_의뢰_정보(String caseManagementNumber, String bankCode) {
        String 기관관리번호 = $("#fa_req_no").getValue();
        String 의뢰번호 = $("#rgst_req_no").getValue();
        String 지점명 = $("#eai_fncl_br_nm").getValue();
        String 담당자명 = $("#applDto > div:nth-child(48) > table > tbody > tr > td:nth-child(3)").text();
        String 담당자_연락처 = $("#applDto > div:nth-child(48) > table > tbody > tr > td:nth-child(4)").text();
        String 메모 = $("#memo").text();
        ...
        caseBasicInfo.put("ACCNT_CD", bankCode);
        ...
        caseBasicInfo.put("SND_DOC_MEMO", String.format("금융기관신청번호 : %s%n말소 : %s", 기관관리번호, 메모));
    ...
    public Map<String, String> 등기_상세_정보() {
        String 등기유형 = $("#applInfo > tbody > tr:nth-child(1) > td:nth-child(2) > center").text();
        String 등기원인 = $("#slt_rgst_caus_text").text();
        String 등기원인_연월일 = $("#rgst_caus_date").getValue();
        String 등기신청일자 = $("#rgst_req_date").getValue();
        String 대출실행일자 = $("#loan_exe_date").getValue();
        String 채권최고액 = $("#applInfo > tbody > tr:nth-child(1) > td:nth-child(6) > input.readonly.price").getValue();
        ...
        caseDetailInfo.put("RGSTRN_RSN_TYP_CD", "5"); // 국민은행은 '자담' 고정
    ...
    public List<Map<String, String>> 담보제공자_정보(String caseManagementNumber) {
        ...
        for (SelenideElement securityProvider : securityProviders) {
            ...
            $("body > div.txt_cen > input.btn_pink.btnsize_normal").click();
    ...
    public Map<String, String> 금융기관_정보() {
        ...
        String 금융기관_등록번호_앞자리 = $("#rgtf_rsdnt_no").getValue().split("-")[0];
        String 금융기관_등록번호_뒷자리 = $("#rgtf_rsdnt_no").getValue().split("-")[1];
