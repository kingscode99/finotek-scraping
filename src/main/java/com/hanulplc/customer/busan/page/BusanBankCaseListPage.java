...
import com.hanulplc.customer.busan.model.BusanBankScrapingDAO;
...
@Slf4j
public class BusanBankCaseListPage implements FinotekCaseListPage {

    public static final String URL = "https://busan.nowrms.com/rgst/appl/list?prog_step=ST_10";

    @Override
    public FinotekCaseListPage scrapeCases() {
        Set<String> caseIds = scrapeCurrentAllCases();
        for (String caseId : caseIds) {
            OptionalInt count = BusanBankScrapingDAO.findByCaseId(caseId);
    ...
    private List<String> scrapeCurrentPageCaseIds() {
        ...
        return currentPageCases.stream()
            .filter(it -> {
                String 물건소재지주소 = it.find(By.tagName("td"), 3).text().replaceAll("(외|외 )(\\d|\\d{2})건", "");
    ...
    private void scrapeCase(String caseId) {
        BusanBankCaseDetailPage caseDetailPage = open(String.format("https://busan.nowrms.com/rgst/appl/edit/%s", caseId), BusanBankCaseDetailPage.class);
        ...
        BusanBankScrapingDAO.saveNewCase(의뢰_기본_정보, 담보제공자_정보, 부동산_정보);
